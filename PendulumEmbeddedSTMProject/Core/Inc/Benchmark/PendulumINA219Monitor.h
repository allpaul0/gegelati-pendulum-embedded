...
class PendulumINA219Monitor: public INA219Monitor {
	...
	PendulumExecutionEnvironment& env;

	/// Record the frame number in the PendulumExecutionEnvironment for each current measure.
...
public:
	 ...
	 * \param[in] env the PendulumExecutionEnvironement used.
	 ...
	 */
	PendulumINA219Monitor(INA219_t * ina219t, PendulumExecutionEnvironment& env, TIM_HandleTypeDef * tim = nullptr, TimeUnit timUnit = TimeUnit::None, float timMultiplier = 0.f);
